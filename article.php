...
<title>Power Hawks - Article</title>
...
<img src="images/latest.png" alt="latest news" /><br />
...
<?php
	...
	else if($_GET[id] < 1 || $_GET[id] > mysql_num_rows(mysql_query("SELECT * FROM latestNews"))) $id = 1;
	else $id = htmlentities($_GET[id]);
	$result = mysql_query("SELECT * FROM latestNews WHERE id = " . $id);
